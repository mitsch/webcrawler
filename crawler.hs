...
mapS :: NFData b => (a -> b) -> [a] -> [b]
mapS _ [] = []
mapS f (x:xs) = let y = f x in y `deepseq` (y : mapS f xs)
...
-- puts queuing reference
putQueuingReference :: ChanClass c => c (OutputCarrier MyOutput) -> URI -> IO ()
putQueuingReference chan u = put chan $ T.intercalate (T.singleton '\t') [T.pack "queue", T.pack $ show u]
...
parser settings responseChannel trackChannel outputChannel = runRing responseChannel trackChannel $ \(u, r) -> do
	...
	then do
		...
		let refs = mapS (\(u', s) -> (u', map (T.pack . L.unpack) s)) $ parse (parserOutputs settings) u body
		(forwardList . map NewSeed) $ filter doLooping $ map fst $ refs
		lift $ putList outputChannel $ mapS (\(l, t) -> (T.pack $ intercalate "\t" $ ["reference", show u, show l]) `T.append` (T.intercalate (T.singleton '\t') t)) $ refs
	...
	= void $ runRingS trackChannel urlChannel (badSuffixes, fromList history, []) $ \ (badSuffixes_, history_, robots_) t -> do
	  	case t of
	  		...
	  		NewSeed u -> do
	  			...
	  			if uText `member` history_ then return (badSuffixes_, history_, robots_)
